...
GPIO.output(GPIO_TRIGGER,True)          #Enviamos um pulso ultrasonico
time.sleep(0.00001)                     #Uma pequena pausa
GPIO.output(GPIO_TRIGGER,False)         #Encerramos o pulso
...
while GPIO.input(GPIO_ECHO)==1:         #Se o sensor recebe sinal...
    stop = time.time()                  #Guarda o tempo atual mediante time.time() em outra variavel


elapsed = stop-start                    #Obtemos o tempo decorrido entre envio e recepcao
distance = (elapsed * 34300)/2          #Distancia e igual ao tempo * velocidade / 2   D = (T x V)/2
...
GPIO.cleanup()                          #Encerra-se o uso das portas GPIO
